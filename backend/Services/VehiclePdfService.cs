...
// Single-page proof-of-booking certificate for Vehicle Booking, generated on demand (not stored)
// once a booking has won its final Approval GA sign-off - see
// BookingKendaraanController.DownloadBuktiPdf. Layout mirrors BookingPdfService (Room Booking's
// own certificate) for consistency, minus the fields Vehicle Booking has no equivalent of (Tipe,
// AdditionalRooms, recurring series).
public static class VehiclePdfService
{
    ...
    public static byte[] Generate(BookingKendaraan item)
    {
        ...
        var document = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Margin(36);
                // Explicit Arial - see BookingPdfService for why (default font drops "ti" ligatures).
                ...
                page.Content().Column(col =>
                {
                    ...
                    col.Item().PaddingTop(8).AlignCenter().Text("BUKTI BOOKING KENDARAAN").Bold().FontSize(15);
                    ...
                    col.Item().PaddingTop(14).Text(
                        "Dokumen ini diterbitkan otomatis oleh sistem PGN Solution (GAAS) sebagai bukti bahwa kendaraan di atas telah disetujui dan dikonfirmasi untuk jadwal yang tercantum. Nomor pemesanan pada dokumen ini dapat digunakan sebagai referensi verifikasi."
    ...
    private static void BuildTable(IContainer container, BookingKendaraan item)
    {
        container.Table(table =>
        {
            ...
            table.Cell().Border(1).BorderColor(BorderColor).Padding(5).Text(item.Keperluan).FontSize(9.5f);
            table.Cell().Border(1).BorderColor(BorderColor).AlignCenter().Padding(5).Text("1").FontSize(9.5f);
            table.Cell().Border(1).BorderColor(BorderColor).AlignCenter().Padding(5).Text("Kendaraan").FontSize(9.5f);
            table.Cell().Border(1).BorderColor(BorderColor).Padding(5).Text(VehicleLabel(item)).FontSize(9.5f);
            table.Cell().Border(1).BorderColor(BorderColor).Padding(5).Text(JamLabel(item)).FontSize(9.5f);
        });
    }

    private static void ExtraInfoBox(IContainer container, BookingKendaraan item)
    {
    ...
    private static void ExtraGrid(IContainer container, BookingKendaraan item)
    {
    ...
    private static string DivisiLabel(BookingKendaraan item)
        => string.IsNullOrEmpty(item.Departemen) ? item.Divisi : $"{item.Divisi} / {item.Departemen}";

    private static string VehicleLabel(BookingKendaraan item)
        => string.IsNullOrEmpty(item.PlatNomor) ? item.NamaKendaraan : $"{item.NamaKendaraan} ({item.PlatNomor})";

    private static string JamLabel(BookingKendaraan item)
