...
using (var scope = app.Services.CreateScope())
{
    ...
    // One-time backfill: every Divisi/Departemen Admin/Approval account's Username switched from
    // the old concatenated-caps convention (e.g. "BadDebtADMIN") to a readable "{Nama} Admin"/
    // "{Nama} Approval"/"{Nama} Admin Div"/"{Nama} Approval Div" form (see OrgTree.cs), and the
    // single Admin/Approval GA accounts became the first of a new "...1"/"...2" pair (see
    // DbSeeder.cs). Guarded with a NOT EXISTS check on the new username - harmless if this ever
    // runs after DbSeeder has already inserted the new-username row directly (e.g. a fresh
    // database seeded straight from the updated OrgTree.cs/DbSeeder.cs), since Username is unique
    // and an unconditional UPDATE would otherwise collide with it. Self-limiting like the en-dash
    // backfill above: once a row is renamed it no longer matches the WHERE clause.
    ...
    // Backfill: simplify competitor conflict reject reason (WHERE-guarded, so it's a no-op once
    // already applied - safe to run on every startup, unlike a plain DELETE would be).
    migrateDb.Database.ExecuteSqlRaw(@"
        UPDATE booking_ruang
    ...
    // Lets the frontend tell an Admin/Approval GA-initiated cancel (shown as "Delete"/"Rejected:
    // <nama>") apart from the origin creator cancelling their own booking (still shown as
    // "Cancelled"). See BookingRuangController.Cancel/BookingKendaraanController.Cancel.
    ...
    if (!migrateDb.MeetingRooms.Any())
    {
        foreach (var room in MeetingRooms.SeedData)
        {
            migrateDb.MeetingRooms.Add(new MeetingRoom
            {
                Nama = room.Nama,
    ...
    if (!migrateDb.Vehicles.Any())
    {
        foreach (var vehicle in Vehicles.SeedData)
        {
            migrateDb.Vehicles.Add(new Vehicle
            {
                Nama = vehicle.Nama,
    ...
    // Loads Rooms/Fleet from the tables just created/backfilled above, same reasoning as
    // OrgTree.LoadFromDb above - every BookingRuangController/BookingKendaraanController call site
