...
public class BookingKendaraanController : ApiControllerBase
{
    ...
    private static (string divisi, string? departemen, RoleEnum createdByRole) EffectiveOwner(User user, BookingKendaraanCreate payload)
    {
    ...
    private static IQueryable<BookingKendaraan> ApplyBulanFilter(IQueryable<BookingKendaraan> query, string? bulan)
    {
        ...
        if (parts.Length == 1)
        {
            ...
            return query.Where(b => b.Tanggal.Year == yearOnly);
        }
        ...
        bool onlyOnApproval = false)
    {
        ...
        {
            // Full access (see IsGaActor/CanAccessBookingKendaraan) - no status or ownership
    ...
    public async Task<IActionResult> Submit(int itemId)
    {
        ...
        if (nextStatus == BookingStatusEnum.APPROVED_GA_APPROVAL)
        {
            ...
            await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item, user.Id), "created", "kendaraan", item.Id, ItemLabel(item), user.Id, user.Nama, user.Role.ToString(), "Mengajukan Booking Kendaraan Baru");
            return Ok(BookingKendaraanOut.From(item));
        }
    ...
    public async Task<IActionResult> ApproveL1(int itemId)
    {
        ...
        await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item, user!.Id), "approval", "kendaraan", item.Id, ItemLabel(item), user!.Id, user!.Nama, user!.Role.ToString(), "Disetujui (Approval Departemen/Divisi)");
        return Ok(BookingKendaraanOut.From(item));
    }
    ...
    public async Task<IActionResult> RejectL1(int itemId, [FromBody] RejectRequest payload)
    {
        ...
        var saveError = await TrySaveChangesAsync(_db);
        if (saveError != null) return saveError;
        await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item, user!.Id), "rejected", "kendaraan", item.Id, ItemLabel(item), user!.Id, user!.Nama, user!.Role.ToString(), "Ditolak (Approval Departemen/Divisi)");
        return Ok(BookingKendaraanOut.From(item));
    }
    ...
    public async Task<IActionResult> ApproveGa(int itemId)
    {
        ...
        var saveError = await TrySaveChangesAsync(_db);
        if (saveError != null) return saveError;
        await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item, user!.Id), "approval", "kendaraan", item.Id, ItemLabel(item), user!.Id, user!.Nama, user!.Role.ToString(), "Disetujui (Admin GA)");
        return Ok(BookingKendaraanOut.From(item));
    }
    ...
    public async Task<IActionResult> RejectGa(int itemId, [FromBody] RejectRequest payload)
    {
        ...
        var saveError = await TrySaveChangesAsync(_db);
        if (saveError != null) return saveError;
        await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item, user!.Id), "rejected", "kendaraan", item.Id, ItemLabel(item), user!.Id, user!.Nama, user!.Role.ToString(), "Ditolak (Admin GA)");
        return Ok(BookingKendaraanOut.From(item));
    }
    ...
    public async Task<IActionResult> ApproveGaApproval(int itemId)
    {
        ...
        await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item, user!.Id), "approved", "kendaraan", item.Id, ItemLabel(item), user!.Id, user!.Nama, user!.Role.ToString(), "Disetujui (Approval GA)");
        return Ok(BookingKendaraanOut.From(item));
    }
    ...
    public async Task<IActionResult> RejectGaApproval(int itemId, [FromBody] RejectRequest payload)
    {
        ...
        var saveError = await TrySaveChangesAsync(_db);
        if (saveError != null) return saveError;
        await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item, user!.Id), "rejected", "kendaraan", item.Id, ItemLabel(item), user!.Id, user!.Nama, user!.Role.ToString(), "Ditolak (Approval GA)");
        return Ok(BookingKendaraanOut.From(item));
    }
