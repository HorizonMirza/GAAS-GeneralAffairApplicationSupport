...
public class PermintaanAtkController : ApiControllerBase
{
    ...
    public async Task<IActionResult> ApproveL1(int itemId)
    {
        ...
        await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item, user!.Id), "approval", "atk", item.Id, ItemLabel(item), user.Nama, "Disetujui (Approval Departemen/Divisi)");
        return Ok(PermintaanAtkOut.From(item));
    }
    ...
    public async Task<IActionResult> RejectL1(int itemId, [FromBody] RejectRequest payload)
    {
        ...
        var saveError = await TrySaveChangesAsync(_db);
        if (saveError != null) return saveError;
        await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item, user!.Id), "rejected", "atk", item.Id, ItemLabel(item), user.Nama, "Ditolak (Approval Departemen/Divisi)");
        return Ok(PermintaanAtkOut.From(item));
    }
    ...
    public async Task<IActionResult> ApproveGa(int itemId, [FromBody] ApproveGaAtkRequest payload)
    {
        ...
        var saveError = await TrySaveChangesAsync(_db);
        if (saveError != null) return saveError;
        await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item, user!.Id), "approval", "atk", item.Id, ItemLabel(item), user.Nama, "Disetujui (Admin GA)");
        return Ok(PermintaanAtkOut.From(item));
    }
    ...
    public async Task<IActionResult> RejectGa(int itemId, [FromBody] RejectRequest payload)
    {
        ...
        var saveError = await TrySaveChangesAsync(_db);
        if (saveError != null) return saveError;
        await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item, user!.Id), "rejected", "atk", item.Id, ItemLabel(item), user.Nama, "Ditolak (Admin GA)");
        return Ok(PermintaanAtkOut.From(item));
    }
    ...
    public async Task<IActionResult> ApproveGaApproval(int itemId)
    {
        ...
        var saveError = await TrySaveChangesAsync(_db);
        if (saveError != null) return saveError;
        await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item, user!.Id), "approval", "atk", item.Id, ItemLabel(item), user.Nama, "Disetujui (Approval GA)");
        return Ok(PermintaanAtkOut.From(item));
    }
    ...
    public async Task<IActionResult> RejectGaApproval(int itemId, [FromBody] RejectRequest payload)
    {
        ...
        var saveError = await TrySaveChangesAsync(_db);
        if (saveError != null) return saveError;
        await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item, user!.Id), "rejected", "atk", item.Id, ItemLabel(item), user.Nama, "Ditolak (Approval GA)");
        return Ok(PermintaanAtkOut.From(item));
    }
    ...
    public async Task<IActionResult> ApproveKpu(int itemId)
    {
        ...
        var saveError = await TrySaveChangesAsync(_db);
        if (saveError != null) return saveError;
        await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item, user!.Id), "approved", "atk", item.Id, ItemLabel(item), user.Nama, "Disetujui (Mitra)");
        return Ok(PermintaanAtkOut.From(item));
    }
    ...
    public async Task<IActionResult> RejectKpu(int itemId, [FromBody] RejectRequest payload)
    {
        ...
        var saveError = await TrySaveChangesAsync(_db);
        if (saveError != null) return saveError;
        await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item, user!.Id), "rejected", "atk", item.Id, ItemLabel(item), user!.Nama, "Ditolak (Mitra)");
        return Ok(PermintaanAtkOut.From(item));
    }
