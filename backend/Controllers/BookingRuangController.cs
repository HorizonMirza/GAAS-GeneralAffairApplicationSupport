...
public class BookingRuangController : ApiControllerBase
{
    ...
    private static string ConflictMessage(BookingRuang conflict) =>
        conflict.IsWholeDay
            ...
            : $"{conflict.NamaRuang} sudah dipesan pada jam {conflict.JamMulai:HH:mm}-{conflict.JamSelesai:HH:mm} di tanggal {conflict.Tanggal:dd/MM/yyyy}";
    ...
    public async Task<IActionResult> Submit(int itemId)
    {
        ...
        if (nextStatus == BookingStatusEnum.APPROVED_GA_APPROVAL)
        {
            ...
            await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item, user.Id), "created", "booking", item.Id, ItemLabel(item), user.Id, user.Nama, "Mengajukan Booking Ruang Baru");
            return Ok(new
            {
    ...
    public async Task<IActionResult> ApproveL1(int itemId)
    {
        ...
        await _db.SaveChangesAsync();
        await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item!, user!.Id), "approval", "booking", item!.Id, ItemLabel(item!), user.Id, user.Nama, "Disetujui (Approval Departemen/Divisi)");
        return Ok(BookingRuangOut.From(item!));
    }
    ...
    public async Task<IActionResult> RejectL1(int itemId, [FromBody] RejectRequest payload)
    {
        ...
        await _db.SaveChangesAsync();
        await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item!, user!.Id), "rejected", "booking", item!.Id, ItemLabel(item!), user.Id, user.Nama, "Ditolak (Approval Departemen/Divisi)");
        return Ok(BookingRuangOut.From(item!));
    }
    ...
    public async Task<IActionResult> ApproveGa(int itemId)
    {
        ...
        await _db.SaveChangesAsync();
        await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item, user!.Id), "approval", "booking", item.Id, ItemLabel(item), user.Id, user.Nama, "Disetujui (Admin GA)");
        return Ok(BookingRuangOut.From(item));
    }
    ...
    public async Task<IActionResult> RejectGa(int itemId, [FromBody] RejectRequest payload)
    {
        ...
        await _db.SaveChangesAsync();
        await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item, user!.Id), "rejected", "booking", item.Id, ItemLabel(item), user.Id, user.Nama, "Ditolak (Admin GA)");
        return Ok(BookingRuangOut.From(item));
    }
    ...
    public async Task<IActionResult> ApproveGaApproval(int itemId)
    {
        ...
        await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item, user!.Id), "approved", "booking", item.Id, ItemLabel(item), user.Id, user.Nama, "Disetujui (Approval GA)");
        return Ok(new
        {
            item = BookingRuangOut.From(item),
    ...
    public async Task<IActionResult> RejectGaApproval(int itemId, [FromBody] RejectRequest payload)
    {
        ...
        await transaction.CommitAsync();
        await _db.Entry(item).ReloadAsync();
        await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item, user!.Id), "rejected", "booking", item.Id, ItemLabel(item), user.Id, user.Nama, "Ditolak (Approval GA)");
        return Ok(BookingRuangOut.From(item));
    }
