...
public class BookingRuangController : ApiControllerBase
{
    ...
    private static (string divisi, string? departemen, RoleEnum createdByRole) EffectiveOwner(User user, BookingRuangCreate payload)
    {
    ...
    // APPROVED_GA_APPROVAL is excluded since it's Room Booking's own true final "Approved" state
    // (no Mitra/KPU stage here), not something still waiting on someone.
        ...
        bool onlyOnApproval = false)
    {
        ...
        {
            // Full access (see IsGaActor/CanAccessBookingRuang) - no status or ownership
    ...
    public async Task<IActionResult> SuperAdminDelete(int itemId)
    {
        ...
        LogDeletion(_db, "booking-ruang", item.Id, item.NomorPemesanan, user!);
        _db.BookingRuangs.Remove(item);
        await _db.SaveChangesAsync();
        return NoContent();
    }
    ...
    public async Task<IActionResult> Submit(int itemId)
    {
        ...
        if (nextStatus == BookingStatusEnum.APPROVED_GA_APPROVAL)
        {
            ...
            await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item, user.Id), "created", "booking", item.Id, ItemLabel(item), user.Id, user.Nama, user.Role.ToString(), "Mengajukan Booking Ruang Baru");
            return Ok(new
            {
    ...
    public async Task<IActionResult> GetStats([FromQuery] string? bulan = null)
    {
        ...
        IQueryable<BookingRuang> query = _db.BookingRuangs.AsQueryable();
        try
        {
    ...
    public async Task<IActionResult> ApproveGaApproval(int itemId)
    {
        ...
        await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item, user!.Id), "approved", "booking", item.Id, ItemLabel(item), user.Id, user.Nama, user.Role.ToString(), "Disetujui (Approval GA)");
        return Ok(new
        {
            item = BookingRuangOut.From(item),
    ...
    public async Task<IActionResult> RejectGaApproval(int itemId, [FromBody] RejectRequest payload)
    {
        ...
        await transaction.CommitAsync();
        await _db.Entry(item).ReloadAsync();
        await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item, user!.Id), "rejected", "booking", item.Id, ItemLabel(item), user.Id, user.Nama, user.Role.ToString(), "Ditolak (Approval GA)");
        return Ok(BookingRuangOut.From(item));
    }
