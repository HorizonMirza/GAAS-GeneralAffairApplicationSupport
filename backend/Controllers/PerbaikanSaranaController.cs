...
public class PerbaikanSaranaController : ApiControllerBase
{
    ...
    public async Task<IActionResult> Submit(int itemId)
    {
        ...
        await _db.SaveChangesAsync();
        await BroadcastActivityNotificationAsync(_hub, await ActivityRecipientIdsAsync(item, user.Id), "created", "sarana", item.Id, ItemLabel(item), user.Nama, "Melaporkan Kerusakan/Pengajuan Perbaikan Baru");
        return Ok(PerbaikanSaranaOut.From(item));
    }
