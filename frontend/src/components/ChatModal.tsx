...
import { CHAT_IMAGE_ACCEPT, MAX_CHAT_IMAGE_SIZE_BYTES, ROLE_COLOR, ROLE_SHORT_LABEL, chatParticipantLabels } from "@/lib/constants";
...
export default function ChatModal({ open, itemId, itemLabel, departemen, createdByRole, me, onClose, onRead }: Props) {
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    setError("");
    try {
      const sent = await api.sendChatMessage(itemId, text, pendingImage);
        ...
        <div className="chat-message-list" ref={listRef}>
          {messages === null ? (
            ...
            messages.map((m, idx) => {
              ...
              const imageUrl = m.hasImage ? api.chatImageUrl(itemId!, m.id) : null;
              return (
                <div
                  key={m.id}
