...
export default function ArsipChatModal({ open, itemId, itemLabel, departemen, createdByRole, me, onClose, onRead }: Props) {
  ...
  useEffect(() => {
    ...
    joinChat("arsip", id).catch((err) => {
      if (!cancelled) setError((err as Error).message);
    });
    const unsubscribe = onChatMessage("arsip", (message) => {
      if (cancelled) return;
      setMessages((prev) => {
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    setError("");
    try {
      const sent = await api.sendArsipChatMessage(itemId, text);
      setMessages((prev) => {
        if (!prev) return [sent];
