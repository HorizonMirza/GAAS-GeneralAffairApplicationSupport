...
import { ROLE_COLOR, getRoleShortLabelMap, bookingChatParticipantLabels } from "@/lib/constants";
...
export default function VehicleBookingChatModal({ open, itemId, itemLabel, departemen, createdByRole, me, onClose, onRead }: Props) {
  ...
  const participantLabels = bookingChatParticipantLabels(departemen, language);
  ...
  useEffect(() => {
    ...
    joinChat("kendaraan", id).catch((err) => {
      if (!cancelled) setError((err as Error).message);
    });
    const unsubscribe = onChatMessage("kendaraan", (message) => {
      if (cancelled) return;
      setMessages((prev) => {
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    setError("");
    try {
      const sent = await api.sendKendaraanChatMessage(itemId, text);
      setMessages((prev) => {
        if (!prev) return [sent];
