...
interface Props {
  ...
  // Same "row-menu-item-danger" treatment as Delete/Cancel below when true - solid red at rest,
  // not just on hover, so it reads the same whether or not the mouse is over it.
...
export default function RowMenuDropdown({
  ...
  return (
    ...
    >
      {onChat && (
        <button type="button" className={`row-menu-item${hasUnreadMention ? " row-menu-item-danger" : ""}`} onClick={onChat}>
