...
// Approve has no notes step (see AtkInvoiceDetailModal.handleApprove) - this modal now only
// handles Reject, which keeps a reason since a rejection needs to explain what's wrong.
export default function AtkInvoiceActionModal({ open, invoiceId, onClose, onDone }: Props) {
  ...
  async function handleConfirm() {
    ...
    setBusy(true);
    try {
      await api.rejectAtkInvoice(invoiceId, value);
  ...
  return (
    <ModalOverlay open={open} onClose={handleClose} className="modal-overlay modal-overlay-centered">
      <div className="modal" style={{ maxWidth: 420 }} ref={containerRef}>
        ...
        <div className="field">
          <label htmlFor="atk-invoice-action-catatan">Catatan (opsional)</label>
          <textarea
            id="atk-invoice-action-catatan"
