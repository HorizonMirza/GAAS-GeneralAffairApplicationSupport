...
export default function RoomMultiSelect({ id, rooms, excludeRoom, selected, onChange, disabled, readOnly }: Props) {
  ...
  return (
    <div className="room-multiselect" ref={wrapRef}>
      ...
      {open && !disabled && (
        <motion.div className="room-multiselect-panel" initial="hidden" animate="visible" variants={sidebarVariants}>
          {readOnly ? (
            selected.length === 0 ? (
              <motion.div variants={itemVariants} className="text-secondary" style={{ fontSize: "0.85rem", padding: "8px 10px" }}>Tidak ada ruangan tambahan</motion.div>
            ) : (
              selected.map((nama) => (
                <motion.div key={nama} variants={itemVariants} className="room-multiselect-option" style={{ cursor: "default" }}>{nama}</motion.div>
              ))
            )
          ) : options.length === 0 ? (
            <motion.div variants={itemVariants} className="text-secondary" style={{ fontSize: "0.85rem", padding: "8px 10px" }}>Tidak ada ruangan lain</motion.div>
          ) : (
            options.map((r) => (
              <motion.label key={r.nama} variants={itemVariants} className="room-multiselect-option">
