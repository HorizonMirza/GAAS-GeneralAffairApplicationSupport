...
export default function SaranaChatModal({ open, itemId, itemLabel, departemen, createdByRole, me, onClose, onRead }: Props) {
  ...
  useEffect(() => {
    ...
    joinChat("sarana", id).catch((err) => {
      if (!cancelled) setError((err as Error).message);
    });
    const unsubscribe = onChatMessage("sarana", (message) => {
      if (cancelled) return;
      setMessages((prev) => {
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    setError("");
    try {
      const sent = await api.sendSaranaChatMessage(itemId, text);
      setMessages((prev) => {
        if (!prev) return [sent];
