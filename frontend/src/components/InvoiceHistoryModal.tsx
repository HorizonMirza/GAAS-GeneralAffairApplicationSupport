...
import { getInvoiceLogActionMeta, getLogRoleLabelMap } from "@/lib/constants";
      ...
      <div className="modal">
        ...
        <div style={{ marginTop: 16 }}>
          {error ? (
            ...
            logs == null ? (
              <p className="text-secondary" style={{ textAlign: "center", padding: "16px 0" }}>{t("history.loading")}</p>
            ) : (
              <p className="text-secondary" style={{ textAlign: "center", padding: "16px 0" }}>{t("history.empty")}</p>
            )
          ) : (
            <div className="approval-log">
              {logs.map((log) => {
                const meta = getInvoiceLogActionMeta(language)[log.action] || { label: log.action, type: "neutral" as const };
                    ...
                    <div className="approval-log-body">
                      ...
                      {log.originalFilename && invoiceId != null && (
                        <div style={{ marginTop: 8, display: "flex", alignItems: "center", gap: 8 }}>
                          ...
                          <a className="btn btn-secondary btn-sm" style={{ width: "auto", padding: "4px 10px", fontSize: "0.75rem" }} href={api.invoiceLogFileUrl(invoiceId, log.id)} target="_blank" rel="noopener noreferrer">{t("common.viewPdf")}</a>
                          <a className="btn btn-secondary btn-sm" style={{ width: "auto", padding: "4px 10px", fontSize: "0.75rem" }} href={api.invoiceLogDownloadUrl(invoiceId, log.id)}>{t("common.downloadPdf")}</a>
                        </div>
                      )}
                    </div>
                  </div>
