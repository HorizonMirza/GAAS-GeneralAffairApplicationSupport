...
export default function AtkChatModal({ open, itemId, itemLabel, departemen, createdByRole, me, onClose, onRead }: Props) {
  ...
  useEffect(() => {
    ...
    joinChat("atk", id).catch((err) => {
      if (!cancelled) setError((err as Error).message);
    });
    const unsubscribe = onChatMessage("atk", (message) => {
      if (cancelled) return;
      setMessages((prev) => {
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    setError("");
    try {
      const sent = await api.sendAtkChatMessage(itemId, text);
      setMessages((prev) => {
        if (!prev) return [sent];
